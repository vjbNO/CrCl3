...
def timeav(runs):
        ...
        for r in runs:
                ...
                print(foldername)
                datafilesUp = np.sort(glob.glob(foldername+'Hexdata_UpperLayer_run_*.txt')) 
                datafilesDown = np.sort(glob.glob(foldername+'Hexdata_LowerLayer_run_*.txt'))
                ...
                np.savetxt('EnsembleAv/TimeAvDown_run_'+str(r)+'.txt',timeavDown)

def ensembleav(runs):
        foldername = './EnsembleAv/'
        datafilesUp = np.sort(glob.glob(foldername+'TimeAvUp_run_*')) #sys.argv[1:]
        datafilesDown = np.sort(glob.glob(foldername+'TimeAvDown_run_*'))
        print(datafilesUp,datafilesDown)
        d0 = np.loadtxt(datafilesUp[0])
        Nx = len(d0)
        Ny = len(d0[0])
