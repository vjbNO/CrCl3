...
def Definecenters(pos):
    ...
    HexPositionsUp = np.mgrid[firstcenterUp[0]:lastcenterUp[0]:HexDistX,firstcenterUp[1]:lastcenterUp[1]:HexDistY,3.75:4:1]
    HexPosOffsetUp = np.mgrid[firstcenterOffsetUp[0]:lastcenterOffsetUp[0]:HexDistX,firstcenterOffsetUp[1]+HexDistY:lastcenterOffsetUp[1]:HexDistY,3.75:4:1]

    HexPositionsDown = np.mgrid[firstcenterDown[0]:lastcenterDown[0]:HexDistX,firstcenterDown[1]:lastcenterDown[1]:HexDistY,-3.75:-3:1]
    HexPosOffsetDown = np.mgrid[firstcenterOffsetDown[0]:lastcenterOffsetDown[0]:HexDistX,firstcenterOffsetDown[1]:lastcenterOffsetDown[1]:HexDistY,-3.75:-3:1]
...
def task(dataname): #what comes in is a singular data file name (string)
    ...
    for i in range(Nspins): #mux is 'key', coords are 'value'. Make mux unique with small marker, because if keys are the same, the item gets replaced!
        data[rawdata[i,0]*(1+1e-15*i)] = (np.round(coorddata[i,0],0),np.round(coorddata[i,1],0),np.round(coorddata[i,2],0))
